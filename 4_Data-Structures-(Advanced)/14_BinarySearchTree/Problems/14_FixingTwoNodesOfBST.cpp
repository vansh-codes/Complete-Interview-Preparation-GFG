...
{
public:
    void findSwappedNodes(Node *root, Node *&prev, Node *&first, Node *&middle, Node *&last)
    {
        if (root == nullptr)
            return;

        findSwappedNodes(root->left, prev, first, middle, last);

        // Check for the first misplaced node
        if (prev && root->data < prev->data)
        {
            ...
            {
                last = root; // Last is the second misplaced node
            }
        }

        prev = root;

        findSwappedNodes(root->right, prev, first, middle, last);
    }
    ...
    void correctBST(Node *root)
    {
        ...
        Node *prev = nullptr, *first = nullptr, *middle = nullptr, *last = nullptr;

        // Find the two misplaced nodes
        findSwappedNodes(root, prev, first, middle, last);
